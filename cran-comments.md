...
* All examples that touch the network are wrapped in `\donttest{}` and in
  `tryCatch()`, so they never fail when the PDET/MTE FTP server is unreachable
  from the check machine. Every other example runs offline on the three small
  sample archives shipped in `inst/extdata` (about 3 KB in total).
